...
func TestRunesReader(t *testing.T) {
	r := NewRunesReader([]rune("0123456789"))
	tests := []struct {
		off     int64
		seek    int
		n       int
		...
		{seek: io.SeekStart, off: -1, seekerr: "Reader.Seek: negative position"},
...
func TestRunesReadAfterBigSeek(t *testing.T) {
	r := NewRunesReader([]rune("0123456789"))
...
func TestRunesReaderAt(t *testing.T) {
	r := NewRunesReader([]rune("0123456789"))
	tests := []struct {
		off     int64
		n       int
		...
		{11, 10, "", io.EOF},
		{0, 0, "", nil},
		{-1, 0, "", "Reader.ReadAt: negative offset"},
	}
	for i, tt := range tests {
		b := make([]byte, tt.n)
...
func TestRunesReaderAtConcurrent(t *testing.T) {
...
func TestRunesEmptyReaderConcurrent(t *testing.T) {
	// Test for the race detector, to verify a Read that doesn't yield any bytes
	// is okay to use from multiple goroutines. This was our historic behavior.
	// See golang.org/issue/7856
	r := NewRunesReader([]rune{})
	var wg sync.WaitGroup
	for i := 0; i < 5; i++ {
		wg.Add(2)
		go func() {
...
func TestRunesReaderWriteTo(t *testing.T) {
	for i := 0; i < 30; i += 3 {
		var l int
		if i > 0 {
			l = len(testString) / i
...
func TestRunesReaderLen(t *testing.T) {
	const data = "hello world"
	r := NewRunesReader([]rune(data))
...
var UnreadRuneErrorTestsRunes = []struct {
	name string
	f    func(*Reader)
}{
...
func TestRunesUnreadRuneError(t *testing.T) {
	for _, tt := range UnreadRuneErrorTestsRunes {
		reader := NewRunesReader([]rune("0123456789"))
...
func TestRunesReaderDoubleUnreadRune(t *testing.T) {
...
func TestRunesReaderCopyNothing(t *testing.T) {
	type nErr struct {
		n   int64
		err error
	}
	...
	with.n, with.err = io.Copy(discard, NewRunesReader(nil))
	withOut.n, withOut.err = io.Copy(discard, justReader{NewRunesReader(nil)})
	if with != withOut {
...
func TestRunesReaderLenSize(t *testing.T) {
	r := NewRunesReader([]rune("abc"))
...
func TestRunesReaderReset(t *testing.T) {
	r := NewRunesReader([]rune("世界"))
